                ...
                <div class="container-fluid table">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <button type="button" style="background-color: #FF9F29; color:white" class="btn btn-danger"
                                disabled id="bulk-presensi" onclick="presensikurban()"
                                style="float: left;">presensikan</button>
                            <a href="{{ route('panitia-kurban.detail', ['key' => 'cetak_presensi', 'event' => $kegiatankurban->event_id]) }}"
                                type="button" style="background-color: #A27B5C; color:white" class="btn btn-danger"
                                style="float: left;">cetak</a>
                            <br>
                        </div>
                        <div class="card-body">
                            ...
                            <div>
                                <table class="table table-striped tablePanitia" id="table-inventaris-book">
                                    <thead class="head-table">
                                        <tr>
                                            <th><input id="check-all" type="checkbox" class="select-all checkbox"
                                                    value="select-all" /></th>
                                            <th>No</th>
                                            <th>Nama Warga</th>
                                            <th>Cabang</th>
                                    ...
                                    <tbody class="body-table">
                                        @foreach ($panitia as $item)
                                            <tr>
                                                <td>
                                                    <input type="checkbox" name="select_process[{{ $item->id }}]"
                                                        class="select_single" value="{{ $item->warga_id }}">
                                                    <input type="hidden" name="warga_id" id="wargaId"
                                                        value="{{ $item->warga_id }}">
                                                </td>
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function() {
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(key, value) {
                            ...
                            result +=
                                ...
                                '<div class="col-6">' +
                                '<input type="hidden" id="warga_id" value=' + value
                                .warga_id + '>' +
                                '<p class="m-0 sub-text" id="warga_id">' + value
                                .warga_id + '</p>' +
                                '<a href=""><h6 class="m-0">' + value.nama +
                                '</h6></a>' +
            ...
            $("#statistik-presensi").load(
                "{{ route('panitia-kurban.detail', ['key' => 'statistik_presensi']) }}&event={{ $item->event_id }}"
                );
